...
def internet_connected(iface=None, n=3, min_percent=30, host='8.8.8.8'):
    '''Check if we're connected to the internet (optionally, check a specific interface `iface`)'''
    try:
        result = subprocess.run(
            "ping {} -c {} {}".format('-I {}'.format(iface) if iface else '', n, host),
            capture_output=True, check=True, shell=True)
        percent = int(re.search(r'(\d+)%\s+\w*\s*loss', result.stdout.decode().split('ping statistics')[-1]).groups()[0])
